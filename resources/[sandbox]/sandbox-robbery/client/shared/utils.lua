...
RegisterNetEvent("Robbery:Client:DetCordFx", function(loc)
	if not HasNamedPtfxAssetLoaded("des_gas_station") then
		RequestNamedPtfxAsset("des_gas_station")
		while not HasNamedPtfxAssetLoaded("des_gas_station") do
			Wait(1)
	...
	UseParticleFxAssetNextCall("des_gas_station")
	SetParticleFxNonLoopedColour(1.0, 0.0, 0.0)
	StartNetworkedParticleFxNonLoopedAtCoord("ent_ray_paleto_gas_explosion", loc.x, loc.y, loc.z, 0.0, 0.0, 0.0, 0.15, false, false, false, false)
	AddExplosion(
		loc.x,
		loc.y,
		loc.z,
	...
	CreateThread(function()
		while c < 5000 do
			StopFireInRange(loc.x, loc.y, loc.z, 50.0)
			Wait(1)
		end
	end)
