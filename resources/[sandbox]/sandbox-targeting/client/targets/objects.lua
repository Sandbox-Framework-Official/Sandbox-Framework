...
TARGETING.AddEntity = function(self, entityId, icon, menuArray, proximity, useObjectIcon)
    ...
    if targetableEntities[entityId] then -- The object has already been added, so add these new menu items to the current menu
        ...
        local targetableObject = targetableObjectModels[GetEntityModel(entityId)]
        if targetableObject and targetableObject.menu then
            for k, v in ipairs(targetableObject.menu) do
                table.insert(currentMenu, v)
            end

            if useObjectIcon then
                targetableEntities[entityId].icon = targetableObject.icon
            end
        end
        ...
        if targetableObject and targetableObject.menu then
            for k, v in ipairs(targetableObject.menu) do
                table.insert(menuArray, v)
            end

            if useObjectIcon then
                icon = targetableObject.icon
            end
        end
