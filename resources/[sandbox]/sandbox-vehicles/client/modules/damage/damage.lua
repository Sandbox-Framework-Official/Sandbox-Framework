...
AddEventHandler("gameEventTriggered", function(event, args)
	if event == "CEventNetworkEntityDamage" then
		...
		if
			DAMAGE_VEHICLE
			and entity == DAMAGE_VEHICLE
			and (NetworkHasControlOfEntity(DAMAGE_VEHICLE) or (DAMAGE_VEHICLE == VEHICLE_INSIDE and VEHICLE_SEAT == -1))
		then
			if weaponHash then -- Reset tank health if vehicle damaged by weapon, so it does not explode lul
				SetVehiclePetrolTankHealth(DAMAGE_VEHICLE, 4000.0)
			end
			if isMelee == 1 then -- Put health back to how it was, you shouldn't be able to set a vehicle on fire by kicking it
				...
				if (engineHealth < LAST_DAMAGE_ENGINE) or (bodyHealth < LAST_DAMAGE_BODY) then
					...
					if bodyHealthDiff >= 200.0 then
						local tire = math.random(0, GetVehicleNumberOfWheels(DAMAGE_VEHICLE))
						SetVehicleTyreBurst(DAMAGE_VEHICLE, tire, true, 0.0)
					end

					if
						INSIDE_HAS_DEGEN and (engineHealthDiff >= 25.0 and engineHealth <= 700.0)
						or (bodyHealthDiff >= 20.0 and bodyHealth <= 850.0)
					then
